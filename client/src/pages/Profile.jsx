...
const Profile = () => {
  const { authUser, updateProfile} = useContext(AuthDataContext)
    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      <div className='w-5/6 max-w-2xl backdrop-blur-2xl text-gray-300 border-2 border-gray-600 flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={ submithandler } className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <label htmlFor="avatar" className='w-fit flex items-center gap-3 cursor-pointer'>
          ...
          <textarea placeholder='Write profile bio...' className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500' rows={4} onChange={(e) => { setbio(e.target.value) }} ></textarea>
          <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 text-white p-2 rounded-full text-lg cursor-pointer' >Save</button>
        </form>
        <img src={authUser?.profilePic || assets.logo_icon} className={`max-w-44 aspect-square rounded-full ${selectedImage && 'rounded-full'} mx-10 max-sm:mt-10`} alt="" />
      </div>
    </div>
  )
}
